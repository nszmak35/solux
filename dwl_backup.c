...
movefloating(const Arg *arg)
{
	...
	// Если окна нет, или оно в фуллскрине — делать нечего
	...
	// 1. Ищем соседа по геометрии центров окон из глобального списка clients
	wl_list_for_each(c, &clients, link) {
		...
		if (arg->ui == 0 && sel_cx <= c_cx) continue; /* Влево */
		if (arg->ui == 1 && sel_cx >= c_cx) continue; /* Вправо */
		if (arg->ui == 2 && sel_cy <= c_cy) continue; /* Вверх */
		if (arg->ui == 3 && sel_cy >= c_cy) continue; /* Вниз */
	...
	// 2. Безопасный SWAP (обмен местами) элементов в wl_list
	if (newsel != NULL) {
		// Проверяем, лежат ли окна в списке прямо друг за другом
		if (sel->link.next == &newsel->link) {
			wl_list_remove(&sel->link);
			wl_list_insert(&newsel->link, &sel->link);
		} else if (newsel->link.next == &sel->link) {
			wl_list_remove(&newsel->link);
			wl_list_insert(&sel->link, &newsel->link);
		} else {
			// Если окна далеко друг от друга в списке, меняем их через временные ссылки
			wl_list_insert(&sel->link, &tmp);
			wl_list_remove(&sel->link);
			wl_list_insert(&newsel->link, &sel->link);
			wl_list_remove(&newsel->link);
			wl_list_insert(&tmp, &newsel->link);
			wl_list_remove(&tmp);
		}

		// Перерисовываем тайлинг
		arrange(selmon);
		
		// Сохраняем фокус на перенесенном окне
		focusclient(sel, 1);
	}
}
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		if (!c && !exclusive_focus &&
			...
			if (launcher == 1) {
				int void_w = TEXTW(selmon, launcher_logo);
				if (cx < void_w) {
					if (event->button == BTN_LEFT) {
						spawn(&(Arg){.v = launcher_exec});
						return;
					}
				}
				x = void_w;	
			...
			else if (cx >= (selmon->b.width - TEXTW(selmon, postlauncher_logo)) && postlauncher == 1 ) {
				if (event->button == BTN_LEFT) {
					spawn(&((Arg){.v = postlauncher_exec}));
					return;
				}
			}
			...
			else if (cx >= (selmon->b.width - TEXTW(selmon, postlauncher_logo) - TEXTW(selmon, postbutton1_logo) - 40)) {
				if (event->button == BTN_LEFT) {
					spawn(&((Arg){.v = (const char*[]){"nm-connection-editor", NULL}}));
					return;
				}
			}
			 */

			else if (cx >= (selmon->b.width - TEXTW(selmon, postlauncher_logo) - 32) && cx < (selmon->b.width - TEXTW(selmon, postlauncher_logo)) && postbutton1 == 1) {
				if (event->button == BTN_LEFT) {
					spawn(&((Arg){.v = postbutton1_exec}));
					return;
				}
			}

			//else if (cx >= (selmon->b.width - TEXTW(selmon, postbutton1_logo) - 40)) {
			//	if (event->button == BTN_LEFT) {
			...
			else if (cx >= (selmon->b.width - TEXTW(selmon, postlauncher_logo) - 65 )  && cx < (selmon->b.width - TEXTW(selmon, postlauncher_logo) - 32) && postbutton2 == 1) {
				if (event->button == BTN_LEFT) {
					spawn(&((Arg){.v = postbutton2_exec}));
					return;
				}
			}
...
drawbar(Monitor *m)
{
	...
	if (launcher == 1) {
		int void_w = TEXTW(m, launcher_logo);
		drwl_setscheme(m->drw, colors[SchemeSel]);
		drwl_text(m->drw, x, 0, void_w, m->b.height, m->lrpad / 2, launcher_logo, 0);
		x += void_w;
	}
	c = focustop(m);
	for (i = 0; i < LENGTH(tags); i++) {
	    ...
	    if (m->tagset[m->seltags] & 1 << i) {
	    	drwl_setscheme(m->drw, colors[SchemeTagSel]);
	        drwl_text(m->drw, x, 0, w, m->b.height, m->lrpad / 2, tags[i], urg & 1 << i);
	        
	        drwl_setscheme(m->drw, colors[SchemeSel]);
	        drwl_rect(m->drw, x, m->b.height - (m->b.height * tagpad), w, m->b.height * tagpad, 1, 0);
	     } else {
	...
	int wifier = TEXTW(m, postbutton1_logo);
	int offner = TEXTW(m, postlauncher_logo);
	int vol_w = TEXTW(m, postbutton2_logo);

	if (postbutton2 == 1) {
		x = m->b.width - offner - wifier - vol_w;
		drwl_setscheme(m->drw, colors[SchemeSel]);
		drwl_text(m->drw, x, 0, vol_w, m->b.height, m->lrpad / 2, postbutton2_logo, 0);	
	}

	if (postbutton1 == 1) {
		x = m->b.width - offner - wifier;
		drwl_setscheme(m->drw, colors[SchemeSel]);
		drwl_text(m->drw, x, 0, wifier, m->b.height, m->lrpad / 2, postbutton1_logo, 0);
	}

	if (postlauncher == 1) {
		x = m->b.width - offner;
		drwl_setscheme(m->drw, colors[SchemeSel]);
		drwl_text(m->drw, x, 0, offner, m->b.height, m->lrpad / 2, postlauncher_logo, 0);
		x += offner;
	}
...
drawstatus(Monitor *m)
{
	...
	//int status_x = m->b.width - tw;
	//x = status_x
	int wifier_w = TEXTW(m, postbutton1_logo);
	int offner_w = TEXTW(m, postlauncher_logo);
	int vol_w = TEXTW(m, postbutton2_logo);
	tw = TEXTW(m, rstext) - m->lrpad;
	if (postbutton1 == 1 && postbutton2 == 1 && postlauncher == 1)
		x = m->b.width - tw - offner_w - wifier_w - vol_w;
	if (postbutton1 == 1 && postbutton2 != 1 && postlauncher == 1)
		x = m->b.width - tw - offner_w - wifier_w;
	if (postbutton1 != 1 && postbutton2 != 1 && postlauncher == 1)
		x = m->b.width - tw - offner_w;
	itext = stext;
...
focusclient(Client *c, int lift)
{
	...
	FILE *f = fopen("/tmp/solux_active_window", "w");
	if (f) {
		if (c) {
	...
	FILE *f_title = fopen("/tmp/solux_active_window_title", "w");
	if (f_title) {
		if (c) {
...
{
	current_kblayout = (current_kblayout + arg->i) % 2;
...
setup(void)
{
	 ...
	 */
	if ((xwayland = wlr_xwayland_create(dpy, compositor, 0))) {
...
dwindle(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i == n - 1) {
			...
			resize(c, (struct wlr_box){nx + gap, ny + gap, nw - 2*gap, nh - 2*gap}, 0);
		} else if (horizontal) {
			int w = nw / 2;

			resize(c, (struct wlr_box){nx + gap, ny + gap, w - gap, nh - 2*gap}, 0);
			...
			resize(c, (struct wlr_box){nx + gap, ny + gap, nw - 2*gap, h - gap}, 0);
...
updatebar(Monitor *m)
{
	...
	wlr_scene_buffer_set_opacity(m->scene_buffer, bar_opacity); //xxxtest
	...
	m->b.height = m->drw->font->height + height_bar;
